...
class PredictedStructureGate(RefoldGate):
    """Self-consistency against a structure predicted elsewhere.

    Running a folding model in-process needs a machine that can host one:
    ``esmfold_v1`` bundles ESM-2 3B and wants roughly 16 GB of RAM. That is a
    deployment constraint, not a reason to skip verification.

    So the check is separable. Fold the designed sequence with whatever you
    have -- ESMFold or AlphaFold on Colab, a cluster, a colleague's GPU --
    save the prediction, and hand the file here. Proteus does the part that
    needs care: superposition and RMSD, which is where the classic mistake
    lives.
    """

    name = "predicted-structure"

    def __init__(self, predicted: str | Structure, plddt: float | None = None,
                 chain: str | None = None, **kw) -> None:
        super().__init__(**kw)
    ...
    def predict(self, sequence: str) -> tuple[np.ndarray, float | None]:
        ...
        structure = self._predicted
        if isinstance(structure, str):
            structure = from_pdb(structure, chain=self._chain)
        plddt = self._plddt
        if plddt is None:
            plddt = _mean_bfactor(structure)
        return structure.coords("ca"), plddt


def _mean_bfactor(structure: Structure) -> float | None:
    """Most predictors write per-residue confidence into the B-factor column.

    AlphaFold and ESMFold both do, on a 0-100 scale. Returns None when the
    column is empty or clearly not a confidence value.
    """
    values = [getattr(r, "bfactor", None) for r in structure]
    values = [v for v in values if v is not None]
    if not values or all(v == 0.0 for v in values):
        return None
    mean = float(np.mean(values))
    if mean <= 1.0:            # some tools write 0-1 instead
        mean *= 100.0
    return mean if 0.0 < mean <= 100.0 else None
